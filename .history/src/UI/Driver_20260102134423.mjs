...
export const Driver = {
    ...
    updateUI(ssot, locale) {
        ...
        // 1. Determine the Best Available Media
        // Logic: Video -> Photo -> SVG Fallback
        const fallback = MediaHelper.getFallbackSVG(State.currentId);
        const photoUrl = ui.tile_photo || "";

        if (ui.video_ref) {
            mediaStage.innerHTML = `
                <video src="${ui.video_ref}" controls autoplay loop muted 
                       onerror="this.parentElement.innerHTML='<img src=\\'${fallback}\\'>'">
                </video>`;
        } else {
            const finalImg = MediaHelper.safeImage(photoUrl, State.currentId);
            mediaStage.innerHTML = `<img src="${finalImg}" alt="Process Preview">`;
        }
