...
async function boot() {
    try {
        ...
        // If ssot is an array, take the first object that has inventory
        if (Array.isArray(ssot)) {
            ssot = ssot.find(item => item.inventory) || ssot[0];
        }
        console.log("Normalized SSOT:", ssot);
        ...
        const products = Object.entries(ssot.REGISTRry)
